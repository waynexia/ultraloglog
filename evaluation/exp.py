...
def prepare(args):
    '''Generates input files used to evaluate the UltraLogLog algorithms.

    Can generate either files containing random 64-bit hashes or a file
...
def plot(args):
    ...
    plt.yticks(np.arange(0, max(means) + max(stdevs)))
    ...
    plt.title('UltraLogLog {} Accuracy (precision: {})'.format(impl, args.precision))
    ...
    plt.savefig(os.path.join(args.output, 'ull-p{}-{}-{}.png'.format(
        args.precision, args.estimator, args.mode)))
    plt.close()
...
def calculate_statistics(location):
    ...
    while True:
        ...
        line_values = [list(map(float, values.split(' ')[0:2])) for values in lines]
...
def parse_args():
    ...
    run_parser.add_argument(
        ...
        type=int,
        default=14,
        help='specify UltraLogLog precision')
